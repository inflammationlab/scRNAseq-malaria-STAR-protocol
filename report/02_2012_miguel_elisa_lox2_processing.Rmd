---
title: 'Report: filtering, dimensional reduction, clustering and marker detection for Lox2 scRNA sample'
...
This report includes a description and the results of the analyses requested by Elisa Jentho at 16/04/2021: 
   ...
   1. Process the Lox2 sample of *Plasmodium chabaudi* scRNA-seq sample with `Seurat` (filtering, dimensional reduction, clustering and marker detection).
...
The processing, filtering, clustering and dimensionality reduction of the 10x Lox2 sample was performed with the `Seurat` R package (v.`r packageVersion("Seurat")`) [@seurat_1; @seurat_2; @seurat_3; @seurat_4].
...
## 10x Lox2 sample: analysis

<br>

### Import 10x Lox2 sample

<br>

The `cellranger` output that resulted from the analysis of the Lox2 sample of *Plasmodium chabaudi* was imported to R with the function `Read10X()` by specifying the files at (folder provided by Elisa Jentho): `data/ftp01.igc.gulbenkian.pt/Lox2_count_full/outs/filtered_feature_bc_matrix`. Then, the data was converted into a `Seurat` class object with the function `CreateSeuratObject()`.
...
# import 10x Lox2 sample
lox2_sample_dir <- "../data/ftp01.igc.gulbenkian.pt/Lox2_count_full/outs/filtered_feature_bc_matrix"
lox2 <- Read10X(data.dir = lox2_sample_dir)

# create Seurat object
lox2_seu <- CreateSeuratObject(counts = lox2, project = "Lox2", min.cells = 3) # 4545 genes x 5204 cells

```

The Lox2 sample before applying any filtering has `r nrow(lox2_seu@assays$RNA@counts)` genes and `r ncol(lox2_seu@assays$RNA@counts)` cells. 
...
vln_plot_qc <- VlnPlot(lox2_seu, features = c("nFeature_RNA", "nCount_RNA"), ncol = 2)

# save 
qc_plots_dir <- "../results/lox2/qc_plots"
...
metrics_2_check <- colnames(lox2_seu@meta.data)[2:3]
...
params_2_filter <- lapply(setNames(metrics_2_check, metrics_2_check), function(x) {
  boxplot.stats(lox2_seu@meta.data[,x])$stats[c(1,5)]
})
...
cells_filtered <- lapply(setNames(metrics_2_check, metrics_2_check), function(x) {
  c( length(which(lox2_seu@meta.data[,x] < params_2_filter[[x]][1])), 
    length(which(lox2_seu@meta.data[,x] > params_2_filter[[x]][2])) )
})
...
# n feature
qc_plots[["boxplots"]][[metrics_2_check[2]]] <- lox2_seu@meta.data %>% 
...
# n count 
qc_plots[["boxplots"]][[metrics_2_check[1]]] <- lox2_seu@meta.data %>% 
...
# n feature
qc_plots[["histograms"]][[metrics_2_check[2]]] <- lox2_seu@meta.data %>% 
  ggplot(data = ., aes(x = nFeature_RNA, fill = orig.ident)) + 
  geom_histogram(binwidth = 50, alpha = 0.5) +
  theme_minimal() + 
  ggsci::scale_fill_npg(name = "Sample") +
  annotate(geom = "text", x = 400, y = 1500,
           label = paste0(750,
                          " (n=", 5185,")"),
           size = 4.5, color = "blue") +
  annotate(geom = "text", x = 900, y = 1500,
           label = paste0(750,
                          " (n=", 19,")"),
           size = 4.5, color = "blue") +
  annotate("rect", xmin = 0, xmax = 750, ymin = 0, ymax = 2000, alpha = .25, fill = "#E64B35FF") +
  annotate("rect", xmin = 750, xmax = 1000, ymin = 0, ymax = 2000, alpha = .25, fill = "#4DBBD5FF") +
...
# n count 
qc_plots[["histograms"]][[metrics_2_check[1]]] <- lox2_seu@meta.data %>% 
  ggplot(data = ., aes(x = nCount_RNA, fill = orig.ident)) + 
  geom_histogram(binwidth = 50, alpha = 0.5) +
  theme_minimal() + 
  ggsci::scale_fill_npg(name = "Sample") +
  annotate(geom = "text", x = 750, y = 750,
           label = paste0(1500,
                          " (n=", 5183,")"),
           size = 4.5, color = "blue") +
  annotate(geom = "text", x = 1900, y = 750,
           label = paste0(1500,
                          " (n=", 21,")"),
           size = 4.5, color = "blue") +
  annotate("rect", xmin = 0, xmax = 1500, ymin = 0, ymax = 1000, alpha = .25, fill = "#E64B35FF") +
  annotate("rect", xmin = 1500, xmax = 2500, ymin = 0, ymax = 1000, alpha = .25, fill = "#4DBBD5FF") +
...
# nCount_vs_nFeature
scatter_feature_plots[["nCount_vs_nFeature"]] <- FeatureScatter(lox2_seu, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") + 
  ...
  annotate("rect", xmin = 0, xmax = 2500, ymin = 750, ymax = 1000, alpha = .25, fill = "#4DBBD5FF") + 
  annotate("rect", xmin = 1500, xmax = 2500, ymin = 0, ymax = 750, alpha = .25, fill = "#4DBBD5FF") +
  annotate("rect", xmin = 0, xmax = 1500, ymin = 0, ymax = 750, alpha = .25, fill = "#E64B35FF") +
  annotate(geom = "text", x = 750, y = 850, label = paste0("(n=5176)"), size = 4.5, color = "blue") +
  annotate(geom = "text", x = 2000, y = 850, label = paste0("(n=28)"), size = 4.5, color = "blue")
...
# apply the filtering
lox2_seu <- subset(lox2_seu, subset = nFeature_RNA < params$max_nFeature & nCount_RNA < params$max_nCount)
...
After applying the filters, from `r nrow(lox2_seu@assays$RNA@counts)` genes it was obtained `r ncol(lox2_seu@assays$RNA@counts)` cells.
...
lox2_seu <- NormalizeData(lox2_seu, normalization.method = "LogNormalize", scale.factor = 10000)
...
Usually 2 K features are chosen for data sets with more than 10 K genes, which represents less than 20% of the genes. Therefore, it was chosen 750 variable features in order to represent ~16% of all variable features for *P. chabaudi* **lox2** sample.
...
lox2_seu <- FindVariableFeatures(lox2_seu, selection.method = "vst", nfeatures = 750)

# retrieve the names of the top30 most abundant to add the names to the plot
top30 <- head(VariableFeatures(lox2_seu), 30)

# plot variable features with and without labels
var_feature_plot <- list()
var_feature_plot[["plot1"]] <- VariableFeaturePlot(lox2_seu)
...
# make dir
plots_folder <- "../results/lox2/plots"
...
The features were scaled prior to dimensional reduction in order to reduce the effect of highly variable and expressed genes/features over others. This was achieved by applying the function `ScaleData()` providing all the genes (n=`r length(lox2_seu@assays$RNA@counts@Dimnames[[1]])`). 
...
# retrive gene names
gene_names <- rownames(lox2_seu) # 4545

# run scaling
lox2_seu <- ScaleData(object = lox2_seu, features = gene_names)
...
# run PCA
lox2_seu <- RunPCA(object = lox2_seu, features = VariableFeatures(object = lox2_seu))
  
# visualize the PCA
dim_pcs_pca_plot <- VizDimLoadings(lox2_seu, dims = 1:4, reduction = "pca")
...
dim_pcs_pca_plot_2[[1]] <- DimPlot(lox2_seu, reduction = "pca", dims = c(1,2))
dim_pcs_pca_plot_2[[2]] <- DimPlot(lox2_seu, reduction = "pca", dims = c(1,3))
dim_pcs_pca_plot_2[[3]] <- DimPlot(lox2_seu, reduction = "pca", dims = c(2,3))
...
pdf(file = paste(plots_folder, "heatmap_dim_reduction_top_20_PCs.pdf", sep = "/"), 
    width = 20, height = 20)
DimHeatmap(lox2_seu, dims = 1:20, cells = 1000, balanced = TRUE, ncol = 4)
dev.off()
...
lox2_seu <- JackStraw(lox2_seu, num.replicate = 100, dims = 50)
lox2_seu <- ScoreJackStraw(lox2_seu, dims = 1:50)

# save 
pdf(file = paste(plots_folder, "jackstraw_plot_top50_PCs.pdf", sep = "/"), 
    width = 14) 
JackStrawPlot(lox2_seu, dims = 1:50)
dev.off()

# plot
print(JackStrawPlot(lox2_seu, dims = 1:50))
...
# plot
elbow_plot <- ElbowPlot(lox2_seu, ndims = 50) + 
  theme_minimal() + 
...
# give the 1st 40 PCs
set.seed(1024)
lox2_seu <- FindNeighbors(lox2_seu, dims = 1:10)
...
for ( res in res_2_iter ) {
  seu <- paste0("lox2_seu_", res)
  set.seed(1024)
  lox2_seu_list[[seu]] <- FindClusters(lox2_seu, resolution = res)
}  

# check how many clusters by resolution
no_clusters <- lapply(lox2_seu_list, function(x) length(levels(x$seurat_clusters)) )
clust_df <- data.frame("Seurat_obj" = names(lox2_seu_list), 
...
It was chosen the resolution value of 0.35 since this gives a number of clusters of 9 similar to what was expected.
...
lox2_seu_umap <- list()
for ( seu in names(lox2_seu_list) ) {
  set.seed(1024)
  lox2_seu_list[[seu]] <- RunUMAP(lox2_seu_list[[seu]], dims = 1:10)
  lox2_seu_umap[[seu]] <- DimPlot(lox2_seu_list[[seu]], reduction = "umap", label = TRUE) + 
    ggtitle(paste0("Resolution: ", clust_df[clust_df$Seurat_obj == seu, "Resolution"]))
}

# plot 
umap_plot_all <- cowplot::plot_grid(plotlist = lox2_seu_umap, ncol = 3)
...
lox2_seu <- lox2_seu_list$lox2_seu_0.35

# retrieve all the markers
set.seed(1024)
lox2_markers_all <- FindAllMarkers(lox2_seu, min.pct = 0.25, logfc.threshold = 0.25)

# retrieve only positive markers
set.seed(1024)
lox2_markers_pos <- FindAllMarkers(lox2_seu, only.pos = TRUE, 
                                 min.pct = 0.25, logfc.threshold = 0.25)

# save all
tables_folder <- "../results/lox2/tables"
if( ! dir.exists(tables_folder) ) dir.create(tables_folder)
write.table(x = lox2_markers_all, file = paste(tables_folder, "all_cluster_markers.tsv", sep = "/"), 
            sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
write.table(x = lox2_markers_pos, file = paste(tables_folder, "positive_cluster_markers.tsv", sep = "/"), 
            sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
```{r echo=FALSE}
datatable(data = lox2_markers_all, extensions = "Buttons", 
...
```{r echo=FALSE}
datatable(data = lox2_markers_pos, extensions = "Buttons", 
...
top10_pos_markers <- lox2_markers_pos %>% 
  group_by(cluster) %>% 
...
top10_markers_clus0 <- VlnPlot(object = lox2_seu, 
...
cowplot::save_plot(filename = paste(plots_folder, "violin_plots_top10_markers_cluster0.pdf", sep = "/"), 
                   plot = top10_markers_clus0, base_height = 14, base_width = 20)
...
top10_markers_clus1 <- VlnPlot(object = lox2_seu, 
...
```{r plot markers - 2, fig.width = 20, fig.height=14}

# top10 markers cluster 2
top10_markers_clus2 <- VlnPlot(object = lox2_seu, 
...
cowplot::save_plot(filename = paste(plots_folder, "violin_plots_top10_markers_cluster2.pdf", sep = "/"), 
                   plot = top10_markers_clus2, base_height = 14, base_width = 20)
...
```{r plot markers - 3, fig.width = 20, fig.height=14}

# top10 markers cluster 3
top10_markers_clus3 <- VlnPlot(object = lox2_seu, 
...
cowplot::save_plot(filename = paste(plots_folder, "violin_plots_top10_markers_cluster3.pdf", sep = "/"), 
                   plot = top10_markers_clus3, base_height = 14, base_width = 20)
...
```{r plot markers - 4, fig.width = 20, fig.height=14}

# top10 markers cluster 4
top10_markers_clus4 <- VlnPlot(object = lox2_seu, 
...
cowplot::save_plot(filename = paste(plots_folder, "violin_plots_top10_markers_cluster4.pdf", sep = "/"), 
                   plot = top10_markers_clus4, base_height = 14, base_width = 20)
...
```{r plot markers - 5, fig.width = 20, fig.height=14}

# top10 markers cluster 5
top10_markers_clus5 <- VlnPlot(object = lox2_seu, 
...
cowplot::save_plot(filename = paste(plots_folder, "violin_plots_top10_markers_cluster5.pdf", sep = "/"), 
                   plot = top10_markers_clus5, base_height = 14, base_width = 20)
...
```{r plot markers - 6, fig.width = 20, fig.height=14}

# top10 markers cluster 6
top10_markers_clus6 <- VlnPlot(object = lox2_seu, 
...
cowplot::save_plot(filename = paste(plots_folder, "violin_plots_top10_markers_cluster6.pdf", sep = "/"), 
                   plot = top10_markers_clus6, base_height = 14, base_width = 20)
...
## Download the latest figure
downloadthis::download_file(
  path = paste(plots_folder, "violin_plots_top10_markers_cluster6.pdf", sep = "/"),
  output_name = "plot",
  button_label = "Download plot",
  button_type = "danger",
  has_icon = TRUE,
...
```{r plot markers - 7, fig.width = 20, fig.height=14}

# top10 markers cluster 7
top10_markers_clus7 <- VlnPlot(object = lox2_seu, 
...
```{r plot markers - 8, fig.width = 20, fig.height=14}

# top10 markers cluster 8
top10_markers_clus8 <- VlnPlot(object = lox2_seu, 
...
cowplot::save_plot(filename = paste(plots_folder, "violin_plots_top10_markers_cluster8.pdf", sep = "/"), 
                   plot = top10_markers_clus8, base_height = 14, base_width = 20)
...
FeaturePlot(lox2_seu, features = genes_interest)
...
# plot heatmap
heatmap_top10_markers <- DoHeatmap(lox2_seu, features = top10_pos_markers$gene) + NoLegend()
...
# create dir
r_objs_folder <- "../results/lox2/R_objects"
if( ! dir.exists(r_objs_folder) ) dir.create(r_objs_folder)

# save R objects
saveRDS(object = qc_plots, file = paste(r_objs_folder, "qc_plots.rds", sep = "/"))
saveRDS(object = lox2_seu_list, file = paste(r_objs_folder, "lox2_seu_list.rds", sep = "/"))
saveRDS(object = lox2_seu, file = paste(r_objs_folder, "lox2_seu.rds", sep = "/"))
